...
function appendChilds(parsedTags, closeTagList) {
    var listWithChilds = {};
    var tags = _.keys(parsedTags);
    console.log(closeTagList);
    console.log(tags);

    
    for (var i = tags.length; i--; ) {
        for (var j = closeTagList.length; j--; ) {
            if(closeTagList[j] === '/' + tags[i] + '>') {
                var child = tags[i],
                    parent = closeTagList[j + 1];
                if(parent !== undefined) {
                    console.log('child: ' + child);
                    if(listWithChilds[parsedTags[parent]] === undefined) {
                    } 
                    if(Array.isArray(parsedTags[child])) {
                        console.log(child + ' is array');
                    }
                    console.log(child + ' is a child of ' + parent);
                }
                break;
            }
        }
